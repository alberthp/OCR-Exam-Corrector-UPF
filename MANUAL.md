...
> (the invented "Albert Einstein" exam), `01` just from a fresh
> `StartScreen()` so its version footer can never go stale again. Re-run
> that script after a UI change (or a version bump) to regenerate all
> eight at once. It needs a real (non-offscreen) Qt platform to render
> text correctly, so run it interactively on a desktop session, not
> headless CI.
