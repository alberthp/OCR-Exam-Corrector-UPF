...
> `01`, `02`, and `04`–`10` are generated by
...
> (the invented "Albert Einstein" exam), `01`/`02` from a fresh
> `StartScreen()`/`NewExamScreen()` (with that same example filled in) so
> neither can drift out of date again. Re-run that script after a UI
> change (or a version bump) to regenerate all nine at once. It needs a
> real (non-offscreen) Qt platform to render text correctly, so run it
> interactively on a desktop session, not headless CI.
