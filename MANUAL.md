...
**Version 1.8** · Albert Hernansanz ([albert.hernansanz@upf.edu](mailto:albert.hernansanz@upf.edu))
...
> by [`tools/capture_manual_screenshots.py`](../tools/capture_manual_screenshots.py) —
> re-run it after a UI change (or a version bump) to regenerate all ten
> at once. It needs a real (non-offscreen) Qt platform to render text
> correctly, so run it interactively on a desktop session, not headless CI.
>
> **⚠️ Past incident**: `03-analysis-running.png` previously showed a real
> exam run and displayed several real students' actual names, DNIs, and
> U-numbers in the live results table — this contradicted the project's
> "don't push real data" policy and had been sitting in git history (and
> on the public GitHub remote) since it was first added. It's now
> generated the same way as the rest: a synthetic multi-page PDF built by
> repeating `examples/scanned_exam_example.pdf`'s one invented page.
> **The exposed real data still exists in git history** on commits before
> this fix — purging it (history rewrite + force-push, and possibly
> contacting GitHub about cached/forked copies) needs a deliberate,
> separate decision, not something to do silently as part of a docs pass.

| File | Screen | What it shows | Status |
| ------ | -------- | ----------- | ------ |
| `01-start-screen.png` | Start | Full window at launch, both main buttons visible, footer showing the current version. | ✅ current (no PII) |
| `02-new-exam-form.png` | New exam | Form with all four required fields filled in (incl. **Exam type**, v1.5), **Run Analysis** green. | ✅ current (no PII) |
